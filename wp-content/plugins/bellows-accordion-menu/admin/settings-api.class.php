 ...
 */
if ( !class_exists( 'Bellows_Settings_API' ) ):
class Bellows_Settings_API {
	...
	function callback_multicheck( $args ) {
		...
		$html = '<div class="ss-multicheck-wrap">';
	...
	function callback_radio_advanced( $args ) {
		...
		foreach ( $options as $group_id => $group_op ) {
			...
			foreach( $group_op as $key => $op_data ){
				...
				$html .= sprintf( '<input type="radio" class="radio" id="%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s"%4$s />', $args['section'], $args['id'], $key, checked( $value, $key, false ) );
				$html .= sprintf( '<label for="%1$s[%2$s][%4$s]"> %3$s</label><br>', $args['section'], $args['id'], $label, $key );
				$html .= sprintf( '<span class="ss-radio-description"> %s</span><br/>', $op_data['desc'] );
			}
		}
		$html .= sprintf( '<span class="description"> %s</label>', $args['desc'] );
	...
	function callback_color_gradient( $args ) {
		...
		$html = sprintf( '<input type="text" class="ssmenu-color-stop"  value="%4$s" data-default-color="%5$s" />', $size, $args['section'], $args['id'], $c1, $args['std'] );
		$html .= sprintf( '<input type="text" class="ssmenu-color-stop"  value="%4$s" data-default-color="%5$s" />', $size, $args['section'], $args['id'], $c2, $args['std'] );
		$html .= sprintf( '<span class="description" style="display:block;"> %s</span>', $args['desc'] );

		$html .= sprintf( '<input type="hidden" class="%1$s-text ssmenu-gradient-list" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s" />', $size, $args['section'], $args['id'], $value, $args['std'] );
...
*/
	...
	function get_option( $option, $section, $default = '' ) {
		...
		return _BELLOWS()->get_default( $option , $section );
...
// 	}
			...
			<div class="postbox">
				<?php foreach ( $this->settings_sections as $form ) { ?>
					<div id="<?php echo $form['id']; ?>" class="group">
						<?php if( isset( $form['custom'] ) ): ?>
							<?php 
								if( function_exists( $form['custom'] ) ) $form['custom']();
								else echo $form['custom'] . '() is not defined'; 
							?>
						<?php else: ?>
						<form method="post" action="options.php">

							<?php /*
							<div class="ss-submit-wrap ss-submit-wrap-top">
							...
							*/ ?>
							...
							<!-- Tabs -->
							<div class="ss-sub-sections">
								<a class="ss-sub-section-tab" data-section-group="_all">Show All</a>
								<?php foreach( $form['sub_sections'] as $section_id => $section_ops ): ?>
								<?php //echo 'do '.$form['id'] . '-' . $section_id; ?>
								<a class="ss-sub-section-tab" data-section-group="<?php echo $section_id; ?>"><?php echo $section_ops['title']; ?></a>
								<?php endforeach; ?>
							</div>							
							
							<div class="ss-form-table-wrap">
								<div class="ss-form-table-wrap-inner">
							...
							<div class="ss-submit-wrap">
								<?php submit_button( 'Save ' . $form['title'] ); ?>
								<span class="ss-submit-tip">(one panel at a time)</span>
							</div>
	...
	function do_settings_fields($page, $section) {
		...
		foreach ( (array) $wp_settings_fields[$page][$section] as $field ) {
			...
			$class = '';
			//$class.= 'ss-field ss-field-'.$this->settings_fields[$section][$k]['type'];
			$class.= 'ss-field ss-field-'.$fields[$k]['type'];
			...
			if( is_array( $group ) ){
				foreach( $group as $g ){
					$class.= ' ss-field-group-'.$g;
				}
			}
			else{
				$class.= ' ss-field-group-'.$group;
			}
	...
	function script() {
		...
		(function($){
			...
			var ssmenu_settings_api_is_initialized = false;

			//jQuery( document ).ready( function($){
			jQuery(function($) {
				initialize_ssmenu_settings_api( 'document.ready' );
			});

			//Backup
			$( window ).on( 'load', function(){
				initialize_ssmenu_settings_api( 'window.load' );
			});

			function initialize_ssmenu_settings_api( init_point ){

				if( ssmenu_settings_api_is_initialized ) return;

				ssmenu_settings_api_is_initialized = true;

				if( console && init_point == 'window.load' ) console.log( 'Bellows Setting API initialized via ' + init_point );
